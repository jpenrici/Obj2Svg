...
/** out_vertices: >= 3 * editor_get_vertex_count(handle) floats, [x0,y0,z0,
 * x1,y1,z1, ...]. */
void editor_get_vertices(EditorHandle handle, float *out_vertices);

size_t editor_get_normal_count(EditorHandle handle);
/** out_normals: >= 3 * editor_get_normal_count(handle) floats, [x0,y0,z0,
 * x1,y1,z1, ...]. */
 ...
 * instead of one call per face.
 */
size_t editor_get_face_count(EditorHandle handle);
/** out_counts: >= editor_get_face_count(handle) uint32_t — vertex/normal count
 * of each face, in order. */
void editor_get_face_vertex_counts(EditorHandle handle, uint32_t *out_counts);
/** Sum of every value editor_get_face_vertex_counts would write — required
 * capacity for the flat buffers below. */
size_t editor_get_face_index_total(EditorHandle handle);
/**
 * out_vertex_indices / out_normal_indices: >=
 * editor_get_face_index_total(handle) uint32_t each; single contiguous buffers,
 * one entry per (face, local vertex) pair in face order — the caller
 * reconstructs per-face offsets by summing editor_get_face_vertex_counts
 * incrementally.
 *
...
/** out_indices: >= 3 * editor_get_triangle_count(handle) uint32_t, [v0,v1,v2,
 * v0,v1,v2, ...] — indices into the same vertex buffer as editor_get_vertices.
 */
...
typedef struct EditorSvgExportOptions {
  bool cull_back_faces;
  bool sort_back_to_front;
  float stroke_width;
  const char *stroke_color;
  const char *fill_color;
} EditorSvgExportOptions;
