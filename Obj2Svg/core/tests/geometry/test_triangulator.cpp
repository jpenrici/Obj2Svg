...
bool approx(float a, float b, float eps = 1e-2f) {
...
float triangle_area_xy(const EditorMesh &mesh, const Triangle &t) {
  ...
  return 0.5f *
         std::fabs((b.x - a.x) * (c.y - a.y) - (c.x - a.x) * (b.y - a.y));
}
...
void test_convex_pentagon() {
  ...
  for (int i = 0; i < n; ++i) {
    const float angle =
        static_cast<float>(i) * 2.0f * static_cast<float>(M_PI) / n;
  ...
  assert(approx(total_area,
                2.3776f)); // regular pentagon, radius 1: (5/2)*sin(72deg)
