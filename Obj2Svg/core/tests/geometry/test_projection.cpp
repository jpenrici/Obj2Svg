// test_projection.cpp
// Simple unit tests for editor_core::{project_mesh, is_face_front_facing}
// — no external framework. Validates the priority behaviors: front/back
// face detection, depth ordering (needed by svg_writer's painter's
// algorithm), the NDC-to-viewport Y flip, and edge deduplication.
...
QuadFixture make_quad_fixture() {
    ...
    fx.camera.projection_matrix = make_identity(); // orthographic for a predictable test
