// test_transform.cpp
// Simple unit tests for editor_core::{Mat4, make_*, multiply, apply_transform}
// — no external framework. Validates only the priority behaviors: each
// primitive transform in isolation, composition order, and the documented
// degenerate-axis edge case.
...
void test_rotation_90_degrees_z() {
    EditorMesh mesh;
    mesh.vertices = {{1.0f, 0.0f, 0.0f}};
    apply_transform(mesh, make_rotation(0.0f, 0.0f, 1.0f, static_cast<float>(M_PI) / 2.0f));
    assert(approx(mesh.vertices[0].x, 0.0f, 1e-3f));
    assert(approx(mesh.vertices[0].y, 1.0f, 1e-3f));
    std::cout << "OK: rotation_90_degrees_z\n";
}

// multiply(a, b) must apply b first, then a — the convention documented in transform.hpp.
void test_composition_order() {
    EditorMesh mesh;
    mesh.vertices = {{1.0f, 0.0f, 0.0f}};
    const auto t = make_translation(2.0f, 3.0f, 4.0f);
    const auto r = make_rotation(0.0f, 0.0f, 1.0f, static_cast<float>(M_PI) / 2.0f);
    apply_transform(mesh, multiply(t, r)); // (1,0,0) --R--> (0,1,0) --T--> (2,4,4)
