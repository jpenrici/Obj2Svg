...
ProjectedMesh make_overlapping_quads_fixture() {
  ...
  mesh.vertices = {
      ...
      {10.0f, 10.0f, 10.0f}, {0.0f, 10.0f, 10.0f}, // far quad: 0-3
      {2.0f, 2.0f, 2.0f},    {8.0f, 2.0f, 2.0f},
      {8.0f, 8.0f, 2.0f},    {2.0f, 8.0f, 2.0f}, // near quad: 4-7
  };
...
void test_wireframe_line_count_and_coordinates() {
  ...
  assert(svg.find("x1=\"0\" y1=\"0\"") !=
         std::string::npos); // vertex 0 = (0,0) appears as an edge endpoint
  assert(svg.find("<polygon") ==
         std::string::npos); // wireframe mode never emits polygons
...
void test_solid_sort_disabled_preserves_original_order() {
  ...
  const auto near_pos =
      svg.find("id=\"polygon-0\""); // now at index 0 after the swap
  const auto far_pos = svg.find("id=\"polygon-1\"");
  assert(near_pos != std::string::npos);
  assert(far_pos != std::string::npos);
  assert(near_pos <
         far_pos); // original (swapped) order preserved, NOT re-sorted by depth
