...
/// @brief Minimal XML attribute escaping for user-supplied color strings.
...
/// @brief Vertex-pair keys of every edge belonging to at least one polygon
///        that passes the @c cull_back_faces filter. Needed because
///        ProjectedMesh.edges is a single, face-agnostic deduplicated set
///        (see project_mesh) — this reconstructs which of those edges
///        remain visible once back faces are (optionally) culled.
std::set<EdgeKey> visible_edge_keys(const ProjectedMesh& mesh, bool cull_back_faces) {
    ...
    for (const auto& poly : mesh.polygons) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            keys.insert(normalize_edge(poly.vertex_indices[i], poly.vertex_indices[(i + 1) % n]));
        }
    }
    return keys;
}

void write_wireframe(std::ostringstream& out, const ProjectedMesh& mesh, const SvgExportOptions& options) {
    const std::set<EdgeKey> visible = visible_edge_keys(mesh, options.cull_back_faces);
...
void write_solid(std::ostringstream& out, const ProjectedMesh& mesh, const SvgExportOptions& options) {
    ...
    if (options.sort_back_to_front) {
        // Farthest first: later SVG elements paint over earlier ones, so
        // emitting back-to-front reproduces the painter's algorithm.
        std::stable_sort(visible.begin(), visible.end(), [&](std::size_t lhs, std::size_t rhs) {
            return mean_depth(mesh, mesh.polygons[lhs]) > mean_depth(mesh, mesh.polygons[rhs]);
        });
    }
...
std::string build_svg(const ProjectedMesh& mesh, const SvgExportOptions& options) {
    ...
    out << "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"" << box.min_x << " " << box.min_y << " "
...
std::expected<void, EditorError> write_svg_file(const EditorContext& ctx, std::string_view filepath,
    ...
    if (!file.is_open()) {
        return std::unexpected(EditorError{
            ErrorCode::IOError, std::string(kCtx),
            "could not open file for writing: " + std::string(filepath), std::nullopt});
    }
    ...
    if (!file) {
        return std::unexpected(EditorError{
            ErrorCode::IOError, std::string(kCtx),
            "failed while writing SVG content to: " + std::string(filepath), std::nullopt});
    }
