...
constexpr float kNormalEpsilon = 1e-7f;
constexpr float kAreaEpsilon = 1e-9f;
...
bool point_strictly_inside(const Point2 &p, const Point2 &a, const Point2 &b,
                           const Point2 &c) {
...
std::vector<Point2>
project_to_dominant_plane(const EditorMesh &mesh,
...
std::expected<TriangleList, EditorError>
triangulate_face(const EditorMesh &mesh,
                 const std::vector<std::size_t> &vertex_indices) {
  ...
  if (n == 3) {
    return TriangleList{
        Triangle{vertex_indices[0], vertex_indices[1], vertex_indices[2]}};
  }

  const std::vector<Point2> points =
      project_to_dominant_plane(mesh, vertex_indices, normal);
  ...
  while (remaining.size() > 3) {
    ...
    for (std::size_t i = 0; i < m; ++i) {
      ...
      for (std::size_t j = 0; j < m; ++j) {
        ...
        if (candidate == prev_local || candidate == cur_local ||
            candidate == next_local) {
          continue;
        }
      ...
      triangles.push_back(Triangle{vertex_indices[prev_local],
                                   vertex_indices[cur_local],
  ...
  triangles.push_back(Triangle{vertex_indices[remaining[0]],
