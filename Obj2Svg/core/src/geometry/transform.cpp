...
Mat4 make_rotation(float axis_x, float axis_y, float axis_z, float angle_radians) {
    ...
    if (len < 1e-8f) {
        return make_identity(); // degenerate axis — documented no-op, see transform.hpp
    }
...
void apply_transform(EditorMesh& mesh, const Mat4& transform) {
    for (auto& v : mesh.vertices) {
        const float x = transform.m[0] * v.x + transform.m[4] * v.y + transform.m[8]  * v.z + transform.m[12];
        const float y = transform.m[1] * v.x + transform.m[5] * v.y + transform.m[9]  * v.z + transform.m[13];
        const float z = transform.m[2] * v.x + transform.m[6] * v.y + transform.m[10] * v.z + transform.m[14];
        v.x = x;
        v.y = y;
        v.z = z;
    }
    // mesh.normals intentionally left untouched — see transform.hpp doc.
}
