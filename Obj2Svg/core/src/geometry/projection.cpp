...
ProjectedVertex project_point(const Vec3 &p, const Mat4 &vp,
                              const Mat4 &view_matrix, int viewport_width,
                              int viewport_height) {
...
ProjectedMesh project_mesh(const EditorMesh &mesh, const CameraView &camera) {
  ...
  for (const auto &v : mesh.vertices) {
    result.vertices.push_back(
        project_point({v.x, v.y, v.z}, vp, camera.view_matrix,
                      camera.viewport_width, camera.viewport_height));
  }

  result.edges.reserve(mesh.faces.size());
...
bool is_face_front_facing(const EditorMesh &mesh, const Face &face,
  ...
  const Vec3 view_normal =
      internal::transform_direction(camera.view_matrix, model_normal);
